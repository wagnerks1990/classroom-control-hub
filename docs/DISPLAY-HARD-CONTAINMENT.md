...
Renderer revision `single-fit-20260909-4` makes text containment a hard invariant.
...
The receiver HTML also uses the same `single-fit-20260909-4` cache key for the layout module and stylesheet. Every renderer behavior change must bump this key; otherwise deployed TVs may continue running stale cached layout code after a Hub update.
...
Expected revision: `single-fit-20260909-4`.
