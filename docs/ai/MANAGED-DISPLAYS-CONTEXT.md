...
11. Display Agent installation and running state are separate. Use `pm path org.classroomhub.display` as the authoritative installed check and `pidof org.classroomhub.display` as the running check. Do not place `dumpsys package` in the critical status path; the validated Onn Android 14 device can block on that command while the package is installed and running.
...
- `adb shell pm path org.classroomhub.display` returned the installed APK path and `adb shell pidof org.classroomhub.display` returned PID `1558` while the UI still showed **Not installed**;
- `dumpsys package org.classroomhub.display` could hang on the device, proving it is unsuitable for the primary agent-status path.
