...
RoomGoblin grants the agent `WRITE_SECURE_SETTINGS`, enables the Android wireless-debugging setting, records the boot policy, switches the current ADB session to port `5555`, and updates the managed endpoint.

## Expected reboot behavior

After boot the Display Agent attempts to restore Android's `adb_wifi_enabled` setting before launching the RoomGoblin display. Pairing authorization should remain separate from this toggle; a supported device should reconnect without another six-digit pairing cycle.

## Security

Use this only on trusted/isolated device-management networks. Never expose the fixed ADB port to the Internet or guest networks. Restrict RoomGoblin maintenance/remote-shell permissions to trusted administrators.
