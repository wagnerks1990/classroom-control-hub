...
Renderer `single-fit-20260909-4` treats configured font sizes as preferences, never as permission to overflow.
...
After updating the Hub, reload receiver pages and verify `window.ClassroomDisplayDiagnostics().revision` reports `single-fit-20260909-4`. The receiver asset cache key is intentionally bumped with each layout behavior change so TVs do not keep stale renderer code.
