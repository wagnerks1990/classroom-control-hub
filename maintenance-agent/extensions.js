...
const ADDONS={
  mosquitto:{id:"mosquitto",name:"MQTT Broker",container:"mosquitto",image:"eclipse-mosquitto:latest",description:"MQTT broker used by Classroom Control Hub integrations."},
  govee2mqtt:{id:"govee2mqtt",name:"Govee Lighting",container:"govee2mqtt",image:"ghcr.io/wez/govee2mqtt:latest",description:"Govee discovery and LAN/cloud control through MQTT."},
  musicassistant:{id:"musicassistant",name:"Music Assistant",container:"music-assistant-server",image:"ghcr.io/music-assistant/server:latest",description:"Optional Classroom Hub-managed audio and media service."},
  veyonwebapi:{id:"veyonwebapi",name:"Veyon WebAPI",container:"veyon-webapi",image:"veyon/webapi-proxy:latest",description:"Optional Veyon WebAPI proxy for classroom workstation control."}
};
...
async function deployAddon(id,settings={},recreate=false){
  ...
  let resolved=settings||{};try{const saved=await mainAppPut(id,settings);resolved=saved.resolved||resolved}catch(error){if(id!=="veyonwebapi")throw error}
  if(exists&&!recreate)return {ok:true,id,adopted:true,managed:true,container:addon.container,message:"Existing container adopted by Classroom Control Hub without recreation."};
  ...
  if(id==="mosquitto"){
    ...
    const base=managedPath("music-assistant");args.push("--network","host","-v",`${base}:/data`);
  }else if(id==="veyonwebapi"){
    // The official proxy image supplies its own HTTP server. Host networking
    // preserves access to classroom Veyon Server endpoints and defaults.
...
express.application.post=function(route,...handlers){
  if(route==="/modules/:id/deploy"&&handlers.length){const original=handlers[handlers.length-1];handlers[handlers.length-1]=async function(req,res,next){const id=String(req.params.id||"");if(!ADDONS[id])return original(req,res,next);try{const result=await deployAddon(id,req.body?.settings||{},req.body?.recreate===true);return res.json(result)}catch(error){return res.status(500).json({ok:false,error:error.message})}};}return originalPost.call(this,route,...handlers)};
