...
app.get("/health",async(_req,res)=>{let docker=false,hostAgent=null;try{await run("docker",["info","--format","{{.ServerVersion}}"],{timeout:5000});docker=true}catch{}try{hostAgent=await hostAgentRequest("GET","/health")}catch(e){hostAgent={ok:false,error:e.message}}res.json({ok:true,version:"1.0.0-alpha.64",docker,hostAgent,roots:{hub:fs.existsSync(HUB_ROOT),services:fs.existsSync(Classroom_ROOT),services:fs.existsSync(Classroom_ROOT)},shellEnabled:ALLOW_SHELL,database:dbStore.databaseInfo()})});
...
app.get("/diagnostics/bundle",async(_req,res)=>{try{
  ...
  const info={createdAt:new Date().toISOString(),agentVersion:"1.0.0-alpha.64",database:dbStore.databaseInfo(),roots:{hub:HUB_ROOT,services:Classroom_ROOT,services:Classroom_ROOT}};
