...
const {JsonStore,DEFAULT_PROFILE,cleanId,cleanHost,cleanPort,cleanSerial,cleanPackage,cleanShell,cleanText,makeId,adbArgsForAction}=require("./android-tv-lib");
...
async function enrollOne(body={}){
  ...
  const device=STORE.upsertDevice({id:body.id||makeId("display"),name:body.name||`${info.manufacturer} ${info.model}`.trim()||serial,host,port:connectPort||Number(serial.split(":").pop()),serial,organization:body.organization,school:body.school,building:body.building,room:body.room,profileId:body.profileId||DEFAULT_PROFILE.id,displayUrl:body.displayUrl,agentPackage:body.agentPackage||"org.classroomhub.display",...info,lastStatus:{online:true,source:"adb-enrollment",checkedAt:new Date().toISOString()}});
...
function installRoutes(app){if(installed)return;installed=true;
  ...
  app.get("/android/devices/:id/status",asyncRoute(async(req,res)=>{const d=findDevice(req.params.id);try{const result=await ensureDevice(d,{mdnsAttempts:8});const updated=STORE.upsertDevice({...result.device,...result.status,lastStatus:{...result.status,source:result.endpoint?"adb-mdns-recovery":"adb-status",checkedAt:new Date().toISOString()}});res.json({ok:true,device:updated,status:result.status,recovered:result.recovered,endpoint:result.endpoint||null})}catch(error){const updated=STORE.upsertDevice({...d,lastStatus:{online:false,error:error.message,checkedAt:new Date().toISOString()}});res.status(503).json({ok:false,error:error.message,device:updated})}}));
  ...
  app.post("/android/devices/:id/agent/install",asyncRoute(async(req,res)=>{let d=findDevice(req.params.id);d=(await ensureDevice(d,{mdnsAttempts:4})).device;const apk=path.resolve(String(req.body?.apkPath||path.join(ROOT,"ClassroomHub-Display-Agent.apk")));if(!(apk===ROOT||apk.startsWith(ROOT+path.sep))||!fs.existsSync(apk)){const e=Error(`Agent APK not found in ${ROOT}. Build/download the signed APK before installation.`);e.status=400;throw e}const r=await adb(["-s",d.serial,"install","-r","-g",apk],{timeout:180000});res.json({ok:true,deviceId:d.id,message:(r.stdout||r.stderr).trim()})}));
  app.post("/android/devices/:id/agent/configure",asyncRoute(async(req,res)=>{let d=findDevice(req.params.id);d=(await ensureDevice(d,{mdnsAttempts:4})).device;const pkg=cleanPackage(req.body?.package||d.agentPackage),url=String(req.body?.displayUrl||d.displayUrl||"").trim();if(!/^https?:\/\//i.test(url)){const e=Error("A valid HTTP(S) display URL is required");e.status=400;throw e}await adb(["-s",d.serial,"shell","am","broadcast","-a","org.classroomhub.display.CONFIGURE","-p",pkg,"--es","display_url",url],{timeout:20000});const updated=STORE.upsertDevice({...d,displayUrl:url,agentPackage:pkg});res.json({ok:true,device:updated})}));
  app.put("/android/devices/:id",asyncRoute(async(req,res)=>{const d=findDevice(req.params.id);res.json({ok:true,device:STORE.upsertDevice({...d,...req.body,id:d.id,host:req.body?.host||d.host,serial:req.body?.serial||d.serial})})}));
