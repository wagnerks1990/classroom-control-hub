...
function applySiteBranding(site={},fromShared=false){
  const product=String(site.productName||'Classroom Control Hub');
...
async function refreshBrandStatus(){
  formatBrandDateTime();
  if(window.brandVersion)brandVersion.textContent=CLASSROOM_HUB_VERSION;
  try{
    ...
    return `<div class="displayOverviewCard">
      ...
      <div class="displayOverviewBody">
        ...
        <div class="toolbar" style="margin-top:10px">
          <button onclick="overviewTestImage('${esc(id)}',${Number(config.avOutput)||0})">Test Image</button>
          ...
          <button onclick="window.open('/display/${encodeURIComponent(id)}?preview=1','_blank')">Preview</button>
        </div>
      </div>
    </div>`;
...
async function clearClassroom(){if(!confirm('Blank ALL enabled classroom displays and pause/clear all lesson queues?'))return;const j=await jpost('/api/v1/classroom/clear-all',{});alert(`Classroom cleared on ${(j.targets||[]).length} display(s).`)}
async function resumeSessions(){await jpost('/api/v1/classroom/resume-sessions',{});loadSession()}
...
function renderPresentationLibrary(){
 ...
 presFolderRows.innerHTML=folders.map(f=>`<div class="card" style="margin:8px 0">
   <div style="display:flex;justify-content:space-between;gap:10px;align-items:center">
     ...
     <div class="toolbar">
       <button onclick="setPresentationFolder('${f.id}')">Open</button>
       <button onclick="renamePresentationFolder('${f.id}')">Rename</button>
       <button onclick="movePresentationFolder('${f.id}')">Move</button>
       <button class="danger" onclick="deletePresentationFolder('${f.id}','${esc(f.name).replace(/'/g,'&#39;')}')">Delete</button>
     </div>
   </div>
 ...
 presentationRows.innerHTML=files.map(p=>`<div class="card" style="margin:8px 0;${PRES.selectedId===p.id?'border-color:var(--green)':''}">
   <div style="display:flex;gap:12px;align-items:center">
     ${p.firstSlideUrl?`<img src="${p.firstSlideUrl}" alt="" style="width:130px;aspect-ratio:16/9;object-fit:contain;background:#000;border-radius:8px">`:''}
     ...
     <div class="toolbar">
       <button class="primary" onclick="selectPresentation('${p.id}')">Select</button>
       <button onclick="renamePresentation('${p.id}')">Rename</button>
       <button onclick="movePresentation('${p.id}')">Move</button>
       ${p.conversionStatus!=='ready'?`<button onclick="rebuildPresentation('${p.id}')">Rebuild</button>`:''}
       <button class="danger" onclick="deletePresentation('${p.id}','${esc(p.name).replace(/'/g,'&#39;')}')">Delete</button>
     </div>
   </div>
...
async function deleteMedia(enc,label){if(!confirm(`Delete "${label}" from Classroom Control Hub? This also removes its generated display PDF.`))return;try{await api('/api/v1/media/'+enc,{method:'DELETE'});loadMedia()}catch(e){alert(e.message)}}
...
function autoTargetValues(){
  ...
  if(action==='tv.power')return [
    ...
    ...Array.from({length:8},(_,i)=>[`tv${i+1}`,`TV ${i+1}`])
  ];
  return [
    ['all','All Displays'],
    ...Array.from({length:8},(_,i)=>[`tv${i+1}`,`TV ${i+1}`])
...
const CLASS_DISPLAY_TARGETS=[
  ['all','All Displays'],
  ...Array.from({length:8},(_,i)=>[`tv${i+1}`,`TV ${i+1}`])
];

function classTargetLabel(id){
  return CLASS_DISPLAY_TARGETS.find(x=>x[0]===id)?.[1]||id;
}
...
function renderClassDefaultTargets(selected=['all']){
  ...
  classDefaultTargets.innerHTML=CLASS_DISPLAY_TARGETS.map(([id,label])=>`
...
function renderClassScheduleList(){classScheduleList.innerHTML=S.classes.length?[...S.classes].sort(compareClassesByPhaseTime).map(x=>`<div class="card" style="margin-bottom:8px"><div class="top"><div><b>${esc(x.name)}</b><div class="muted">${x.startTime}–${x.endTime} • ${esc(classDaySummary(x))} • Displays: ${esc(classTargetSummary(x.defaultTargets))}</div></div><div class="toolbar"><button onclick="editClassSchedule('${x.id}')">Edit</button><button onclick="duplicateClassSchedule('${x.id}')">Duplicate</button><button class="danger" onclick="deleteClassSchedule('${x.id}')">Delete</button></div></div></div>`).join(''):'<div class="muted">No classes configured.</div>'}
...
function populateAutomationClassSelect(selected=[]){
  ...
  autoClassIds.innerHTML=[...S.classes].sort(compareClassesByPhaseTime).map(cls=>`<label><input type="checkbox" data-autoclass="${cls.id}" ${selectedIds.includes(cls.id)?'checked':''} onchange="renderAutomationClassBinding()"> ${esc(cls.name)} <span class="muted">(${esc(classDaySummary(cls))} ${cls.startTime}–${cls.endTime})</span></label>`).join('')||'<span class="muted">No classes configured.</span>';
}
  ...
  if(action==='tv.power'||action==='govee.power'){
    ...
    h=payloadInput('Website / URL','autoUrl','url',payload.url||location.origin+'/')+
      `<label style="display:flex;gap:8px;align-items:flex-start;margin:10px 0">
        ...
        <span class="muted">Use the selected TV's local network connection to open this URL. Classroom Control Hub only sends the URL to the TV; the Hub server does not fetch the page.</span></span>
      </label>`;
...
function stepPayloadHtml(step,i){
  ...
  if(a==='display.media'){
    ...
    <div class="muted">Choose an image, video, PDF, presentation, or document already uploaded to Classroom Control Hub. You no longer need the internal stored filename.</div>`;
  }
  ...
  automationList.innerHTML=sorted.map(e=>{
    ...
    return `<div class="card" style="box-shadow:none;margin:8px 0">
      <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start">
        ...
        <div class="toolbar"><button onclick="editAutomation('${e.id}')">Edit</button><button onclick="duplicateAutomation('${e.id}')">Duplicate</button><button onclick="runAutomation('${e.id}')">Run Now</button><button class="danger" onclick="deleteAutomation('${e.id}','${esc(e.name).replace(/'/g,"&#39;")}')">Delete</button></div>
      </div>
    </div>`;
  }).join('');
}
...
async function loadSession(){try{sessionRaw.textContent=JSON.stringify(await api('/api/v1/sessions/'+encodeURIComponent(sessionId.value)),null,2)}catch(e){sessionRaw.textContent=e.message}}
...
async function loadDiagnostics(){
  try{
    ...
    diagSummary.innerHTML=[
      diagBadge(!!sv.classroomHub?.ok,'Classroom Control Hub',x.system?.version||''),
...
// Classroom Control Hub 1.0 alpha.5 database-native structured configuration
let ADMINCFG=null;
...
function applyAuthUi(j){window.AUTH_STATUS=j||{};const u=j?.user;if(u){authAccount.style.display='block';authAccountName.textContent=u.displayName||u.username;authAccountRole.textContent=(u.role||'').toUpperCase();accountIdentity.textContent=`${u.displayName||u.username} • ${u.username} • ${u.role}`;document.querySelectorAll('button[data-page="settings"],button[data-page="system"]').forEach(b=>b.style.display=u.role==='admin'?'':'none')}else{authAccount.style.display='none'}}
...
async function logoutOtherSessions(){if(!confirm('Sign out every other Classroom Control Hub session for this account?'))return;try{const j=await api('/api/v1/auth/logout-others',{method:'POST'});alert(`Signed out ${j.removed||0} other session(s).`);await loadMySessions()}catch(e){alert(e.message)}}
...
async function toggleLocalAuth(){try{const current=!!window.ADMIN_USERS?.authEnabled;if(current&&!confirm('Disable local Classroom Control Hub authentication? External access controls may still protect the site.'))return;if(!current&&!confirm('Enable local Classroom Control Hub authentication? You will need an administrator account to continue managing the system.'))return;await api('/api/v1/admin/auth',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({enabled:!current})});await loadAdminUsers()}catch(e){alert(e.message)}}
checkAuthentication();
async function loadAdminSecrets(){try{const j=await api('/api/v1/admin/secrets');cfgSecrets.innerHTML=`<table><thead><tr><th>Name</th><th>Purpose</th><th>Updated</th><th></th></tr></thead><tbody>${(j.secrets||[]).map(x=>`<tr><td style="text-align:left"><code>${esc(x.name)}</code></td><td style="text-align:left">${esc(x.metadata?.integration||x.metadata?.type||'Encrypted secret')}</td><td>${esc(new Date(x.updatedAt).toLocaleString())}</td><td><button class="danger" onclick="deleteAdminSecret('${esc(x.name).replace(/'/g,"\'")}')">Delete</button></td></tr>`).join('')}</tbody></table>`}catch(e){cfgSecrets.innerHTML=`<div class="bad">${esc(e.message)}</div>`}}
...
// Classroom Control Hub 1.0 System Management
// -----------------------------------------------------------------------------
let fileManagerPath='.';
...
async function createManagedBackup(){const scope=backupScope.value;if(scope==='full'&&!confirm('Full Recovery backups contain credentials and the encryption master key. Create it anyway?'))return;try{const j=await maintApi('/backup/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({scope,confirmSecrets:scope==='full'})});alert(`Backup created: ${j.name}`);await loadManagedBackups()}catch(e){alert(e.message)}}
...
function renderBgmFavorites(){
  ...
  bgmFavorites.innerHTML=BGM_FAVORITES.length?BGM_FAVORITES.map(f=>`<div class="card" style="box-shadow:none;margin-bottom:8px"><div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><div><b>${esc(f.name)}</b><div class="muted">${esc(f.detail||'')}</div><div class="muted" style="font-size:11px">${esc(f.uri)}</div></div><div class="toolbar"><button class="primary" onclick="bgmPlayFavorite('${esc(f.id)}')">Play</button><button onclick="bgmUseFavorite('${esc(f.id)}')">Use for Schedule</button><button class="danger" onclick="deleteBgmFavorite('${esc(f.id)}')">Remove</button></div></div></div>`).join(''):'<div class="muted">No favorites saved yet. Search Music Assistant below and choose Save Favorite.</div>';
}
...
function openMusicAssistantUi(){const u=(maUrl.value||'http://host.docker.internal:8095').trim();window.open(u,'_blank')}
...
const MODULE_FIELDS={
  mosquitto:[
    {key:'port',label:'MQTT Port',type:'number',default:'1883',help:'TCP port published on the Classroom Control Hub host.'},
...
async function loadManagedModules(){
  try{
    ...
    managedModules.innerHTML=modules.map(m=>{
      ...
      return `<div class="card" style="box-shadow:none;margin-bottom:10px"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start"><div><b>${esc(m.name)}</b><div class="muted">${esc(m.description||'')}</div><div class="muted">Container: ${esc(m.container)} • Image: ${esc(m.image)}</div><div style="margin-top:6px"><span class="pill ${m.state==='running'?'ok':''}">${esc(m.state)}</span> <span class="pill">${esc(label)}</span>${m.composeProject?` <span class="pill">Compose: ${esc(m.composeProject)}</span>`:''}</div></div><div class="toolbar">${actions}</div></div>${m.externalOnly?'':`<details style="margin-top:10px" ${m.configured?'open':''}><summary>Configuration</summary><div id="module-editor-${esc(m.id)}" class="grid2" style="margin-top:10px">${fields.map(f=>moduleFieldHtml(m.id,f,cfg)).join('')||'<div class="muted">No configurable settings.</div>'}</div><div class="muted" style="margin-top:8px">Secret fields are encrypted in the Classroom Control Hub database. A masked value is preserved unless replaced.</div></details>`}</div>`
