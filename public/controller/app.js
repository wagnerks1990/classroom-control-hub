...
const CLASSROOM_HUB_VERSION='1.0.0-alpha.70';
...
async function loadManagedBackups(){try{
  ...
  html+=`<table><thead><tr><th>Backup</th><th>Size</th><th>Created</th><th>Actions</th></tr></thead><tbody>${items.map(x=>`<tr><td style="text-align:left">${esc(x.name)}</td><td>${fmtBytes(x.size)}</td><td>${esc(new Date(x.modifiedAt).toLocaleString())}</td><td><div class="toolbar"><button onclick="inspectManagedBackup('${esc(x.name).replace(/'/g,"\\'")}')">Inspect</button><button onclick="restorePlanManagedBackup('${esc(x.name).replace(/'/g,"\\'")}')">Restore Plan</button>${managedBackupRestoreButtons(x)}<button onclick="window.open('/api/v1/maintenance/backup/${encodeURIComponent(x.name)}','_blank')">Download</button><button class="danger" onclick="deleteManagedBackup('${esc(x.name).replace(/'/g,"\\'")}')">Delete</button></div></td></tr>`).join('')}</tbody></table>`;
