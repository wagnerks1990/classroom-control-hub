...
async function createManagedBackup(scope=backupScope.value,{download=false}={}){const sensitive=scope!=='diagnostic',full=scope==='full';if(sensitive&&!confirm(full?'A Full Recovery Export contains private appliance data, device identity, user or student records, and endpoint credentials. Store the downloaded archive as a secret. Continue?':'This recovery export can contain private appliance data, device identifiers, user or student records, or endpoint credentials. Store it as a secret. Continue?'))return;if(full&&!confirm('The Full Recovery Export also contains protected recovery secrets, including the database encryption master key. Anyone with this archive may be able to reconstruct the appliance. Create it?'))return;try{const j=await maintApi('/backup/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({scope,confirmSensitiveData:sensitive,confirmSecrets:full})});await loadManagedBackups();if(download)downloadManagedBackup(j.name,true,true);alert(`${full?'Full Recovery Export':'Export'} created: ${j.name}${download?'\nDownload started.':''}${j.containsSensitiveData?'\nProtect this sensitive archive.':''}`)}catch(e){alert(e.message)}}
function createFullRecoveryExport(){return createManagedBackup('full',{download:true})}
...
function managedBackupRestoreButtons(item){const arg=inlineJsArg(item.name),modes=new Set(item.restoreModes||[]),exportOnly=item.capabilities?.services||item.capabilities?.secrets;return `${modes.has('configuration')?`<button onclick="restoreManagedBackup(${arg},'configuration')">Restore Database</button>`:''}${modes.has('configuration-data')?`<button class="danger" onclick="restoreManagedBackup(${arg},'configuration-data')">Restore Database + RoomGoblin Data</button>`:''}${modes.size?'':'<span class="muted">No application restore available</span>'}${exportOnly?'<span class="muted">Secrets/services are export-only here</span>':''}`}
async function loadManagedBackups(){try{
  ...
  html+=`<table><thead><tr><th>Backup</th><th>Bundle Contents</th><th>Size</th><th>Created</th><th>Available Actions</th></tr></thead><tbody>${items.map(x=>{const arg=inlineJsArg(x.name);return `<tr><td style="text-align:left">${esc(x.name)}<br><span class="pill ${x.containsSensitiveData?'bad':'ok'}">${x.containsSensitiveData?'Sensitive recovery data':'Support-safe diagnostic'}</span></td><td>${managedBackupCapabilities(x)}</td><td>${fmtBytes(x.size)}</td><td>${esc(new Date(x.modifiedAt).toLocaleString())}</td><td><div class="toolbar"><button onclick="inspectManagedBackup(${arg})">Inspect</button><button onclick="restorePlanManagedBackup(${arg})">View Restore Capabilities</button>${managedBackupRestoreButtons(x)}<button onclick="downloadManagedBackup(${arg},${x.containsSensitiveData===true})">Download</button><button class="danger" onclick="deleteManagedBackup(${arg})">Delete</button></div></td></tr>`}).join('')}</tbody></table>`;
...
function downloadManagedBackup(name,sensitive,confirmed=false){if(sensitive&&!confirmed&&!confirm('This backup contains sensitive recovery data. Keep it encrypted at rest and do not attach it to support tickets. Download anyway?'))return;const link=document.createElement('a');link.href=`/api/v1/maintenance/backup/${encodeURIComponent(name)}`;link.download=name;document.body.appendChild(link);link.click();link.remove()}
